...
message(paste(
  "Starting SSN benchmarking/validation for network", nw, 
  "with", nobs, "observations for rep", rep, "of", nreps
))

# message("Both SSN and S3N, with preprocessing and estimation for both models")

message(paste("LSN path:", lsn_path))
...
tic("SSN code overall")
if(!is.na(nreps_SSN)){
  benchmark_SSN_one_rep(
    nw, nobs, rep, nreps,
    out_dir = bench_res_dir, 
    lsn.path = lsn_path
  )
...
message(paste0(
  "SSN simulation is done for network ", nw, 
